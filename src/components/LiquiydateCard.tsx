...
export default function LiquiydateCard({
  projectId,
  title,
  subtitle,
  endTime,
  ...
  useEffect(() => {
    const getNft = async () => {
      const nftContractId = "vault_" + selectNft + ".liquid-nft-3.testnet"
  ...
  const handleDepositNFT = async () => {
    ...
    if (nftInfo.near_amount > 0 && !nftInfo.near_deposited) {
     transactions.push({
      signerId: wallet.accountId,
    ...
    for (let i = 0; i < nftInfo.token_amounts.length; i++) {  
      if (nftInfo.token_amounts[i] > 0 && !nftInfo.token_deposited[i]) {
        transactions.push({
          signerId: wallet.accountId,
          receiverId: nftInfo.token_contract_ids[i],
          actions: [
            {
              type: "FunctionCall",
              params: {
          ...
          actions: [
            {
              ...
              params: {
                ...
                args: {
                  ...
                  amount: nftInfo.token_amounts[i],
                  msg: "Deposit token"
                },
                gas: "60000000000000",
                deposit: "1"
    ...
    >
      ...
      <VStack width="100%" spacing="24px">
        <Flex minWidth="100%" minHeight="14" justifyContent="center">
          ...
          >
            <Text fontSize="4xl">{isActivated ? "FORGE" : "LIQUIDATE"}</Text>
          </Button>
        </Flex>
      </VStack>
    </Flex>
